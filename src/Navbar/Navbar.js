...
class Navbar extends React.Component {
  ...
  render() {
    const { selectedAddressingFilter, selectedGroup, selectedSite } = this.state;
    const { addressingFilters, groups, sites, viewModes } = this.props;

    return (
      <div className='ap-navbar'>
        <DropdownItem options={addressingFilters} value={selectedAddressingFilter} onSelect={this.handleAddressingSelection} />
        {
          selectedAddressingFilter.value === 1 &&
          <DropdownItem options={sites} value={selectedSite} onSelect={this.handleGroupSelection}/>
        }
        {
           selectedAddressingFilter.value === 2 &&
          <DropdownItem options={groups} value={selectedGroup} onSelect={this.handleGroupSelection} />
        }
